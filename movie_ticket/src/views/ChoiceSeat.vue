<template>
  <div class="content" :style="{height:maxHeight+'rem'}">
    ...
    <div class="seatwindow">
      ...
      <div>
        <span class="iconfont icon-shafa" style="color: white"></span>
        可选
        <span class="iconfont icon-shafa" style="color: #999"></span>
        已售
        <span class="iconfont icon-shafa" style="color: #DF5C4D;"></span>
        已选
        <span class="iconfont icon-shafa" style="color: orange"></span>
        线上专享
      </div>
          ...
          <div v-for="(item,index) in roomSeat" :key="index">
            <span v-for="(seat_item,i) in item" :key="i">
              ...
              <i
                ...
                :data-select="1"
                :data-id="0"
                :class="{xianshang:seat_item==2,yishouchu:seat_item==1}"
              ></i>
            </span>
          </div>
        </div>
    ...
    <div class="showseat">
      <div v-for="(item,index) in showSeats" :key="index">
        <p>
          {{item.x}}排{{item.y}}座
          <i @click="deleteSelect(item.id,$event)">×</i>
        </p>
      </div>
    </div>
    <div>
        ...
        if (result.data.length > 0) {
          ...
          for (let i = 0; i < result.data.length; i++) {
            selectedArr = selectedArr.concat(
              JSON.parse(result.data[i].choiceSeat)
            );
  ...
  methods: {
    ...
    selectSeat(e) {
      ...
      if (this.choiceSeat.length >= 6) {
        if (
          ...
          setTimeout(() => {
            this.$refs.over.style.display = "none";
          }, 2500);
        }
      } else {
        if (
    ...
    nextSteap() {
      if (this.choiceSeat.length == 0) {
        ...
        setTimeout(() => {
          this.$refs.nullseat.style.display = "none";
        }, 1000);
      } else {
        let params = {};
...
.content > div:nth-child(2) > div > p:nth-child(2),
.content > div:nth-child(2) > div > p:nth-child(3) {
  font-size: 1.1rem;
  color: #888;
}
.content > .seatwindow {
...
.content > .seatwindow > .viewport > .over,
.content > .seatwindow > .viewport > .isnull {
  width: 100%;
  height: 100%;
  position: absolute;
  left: 0;
...
.content > .seatwindow > .viewport > .over > div,
.content > .seatwindow > .viewport > .isnull > div {
  position: absolute;
  left: 10rem;
  top: 7rem;
  z-index: 999;
